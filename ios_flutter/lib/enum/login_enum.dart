...
enum RegistState {
 ...
 waitEmailVerify('Email尚未驗證！請收信並驗證');
    final String path;
  const RegistState(this.path);


  Widget buildWidgetByRegistState(List<TextFieldData>  listInfo) {

 String email = listInfo[0].controller.text;
 String pw = listInfo[1].controller.text;

  switch (this) {
    case RegistState.waitRegist:
      return  waitRegistWidget();
    case RegistState.aleardy:
      return  waitAlearyWidget(email);
    
    case RegistState.waitEmailVerify:
      return  waitEmailVerifyWidget();
    case RegistState.success:
